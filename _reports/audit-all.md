...
5. **HIGH — CI is bypassable.** `dorny/paths-filter` defaults to `predicate-quantifier: some`, so any PR touching one `.md`/`.mdx` file skips lint, format, typecheck, and build while reporting green — verified against commit `26407b5` (2 md + 117 ts files). → codebase F2, docs F5.
...
6. Fix `ci.yml`'s `paths-filter` to `predicate-quantifier: every` so a docs-only skip cannot skip code gates. [P5]
